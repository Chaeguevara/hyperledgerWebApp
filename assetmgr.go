...
//Define asset. Define organization asset information and will be traced in blockchain
type OrgAsset struct {
	Id        string `json:"id"`        // Asset ID
	AssetType string `json:"assetType"` //type of device
	Status    string `json:"status"`    // status of aset
...
//define init and invoke methods
...
func (c *AssetMgr) query(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	ENTITY = args[0]
	Avalbytes, err := stub.GetState(ENTITY)
	if err != nil {
